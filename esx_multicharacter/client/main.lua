...
if ESX.GetConfig().Multichar then
    ...
    SetupCharacter = function(index)
        if not spawned then
            exports.spawnmanager:spawnPlayer({x = SpawnCoords.x, y = SpawnCoords.y, z = SpawnCoords.z, heading = SpawnCoords.w, model = Characters[index].model or mp_m_freemode_01, skipFade = true}, function()
                canRelog = false
    ...
    AddEventHandler("esx_multicharacter:SetupUI", function(data, slots)
        ...
        if not Character then
            SendNUIMessage({action = "closeui"})
            exports.spawnmanager:spawnPlayer({x = SpawnCoords.x, y = SpawnCoords.y, z = SpawnCoords.z, heading = SpawnCoords.w, model = mp_m_freemode_01, skipFade = true}, function()
                canRelog = false
                DoScreenFadeIn(400)
                Wait(400)
                local playerPed = PlayerPedId()
